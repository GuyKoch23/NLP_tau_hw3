...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_doc = doc[:length]
        doc_len = round(len(cur_doc) / 4)
        mask = doc_len + random.randint(-1 * doc_len, doc_len)
        id = random.randint(0, mask)
        prefix = cur_doc[:id]
        content = cur_doc[id:id + mask]
        suffix = cur_doc[id + mask:]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + content
        masked_string += self.PAD_CHAR * (self.block_size - len(masked_string))
        input_string = masked_string[:-1]
        output_string = masked_string[1:]
        input_tensor = torch.tensor([self.stoi.get(c) for c in input_string], dtype=torch.long)
        output_tensor = torch.tensor([self.stoi.get(c) for c in output_string], dtype=torch.long)
